...
def execute(filters=None):
	...
	em_id = get_employee_id()

	data = []
	for ss in salary_slips:
		row = [ss.employee_id ,em_id.get(ss.employee),ss.name, ss.employee, ss.employee_name, doj_map.get(ss.employee), ss.branch, ss.department, ss.designation,
...
def get_columns(salary_slips):
	...
	"""
	columns = [
		_("General number"),_("Employee id"),_("Salary Slip ID") + ":Link/Salary Slip:150",_("Employee") + ":Link/Employee:120", _("Employee Name") + "::140",
		_("Date of Joining") + "::80", _("Branch") + ":Link/Branch:120", _("Department") + ":Link/Department:120",
		_("Designation") + ":Link/Designation:120", _("Company") + ":Link/Company:120", _("Start Date") + "::80",
		_("End Date") + "::80", _("Leave Without Pay") + ":Float:130", _("Payment Days") + ":Float:120"
	]
